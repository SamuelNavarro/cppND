        ...
        if (timeSinceLastUpdate >= cycleDuration)
        {
            ...
            if (completion >= 0.9 && !hasEnteredIntersection)
            {
                ...
                // Then, wait for the data to be available before proceeding to slow down.
						    std::future<void> tsk = std::async(&Intersection::addVehicleToQueue, _currDestination, get_shared_this());



								// To wait until entry has been granted. 
								tsk.wait(); // Is this the Then, wait for the data to be available ... line?
